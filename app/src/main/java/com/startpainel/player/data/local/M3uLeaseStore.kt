...
class M3uLeaseStore(private val context: Context) {
    ...
    // Código e URL M3U são criptografados em repouso (SecretBox / Android Keystore).
    val lease: Flow<M3uLease?> = context.m3uDataStore.data.map { prefs ->
        val code    = SecretBox.decrypt(prefs[KEY_CODE]).orEmpty()
        val leaseId = prefs[KEY_LEASE_ID] ?: -1
        if (code.isBlank() || leaseId < 0) null
        else M3uLease(
            code      = code,
            leaseId   = leaseId,
            m3uUrl    = SecretBox.decrypt(prefs[KEY_M3U_URL]).orEmpty(),
    ...
    suspend fun save(lease: M3uLease) {
        context.m3uDataStore.edit { prefs ->
            prefs[KEY_CODE]      = SecretBox.encrypt(lease.code)
            prefs[KEY_LEASE_ID]  = lease.leaseId
            prefs[KEY_M3U_URL]   = SecretBox.encrypt(lease.m3uUrl)
